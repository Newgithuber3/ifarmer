...
{{item.phone}}
      ...
      <el-footer>
        <div class="footer">
          <div class="payment">
            <!--            <div class="truePay">实付款：</div>-->
            实付款：<span style="color: red;">{{totalMoney|formatMoney}}</span>元
          </div>
          <a href="/cart" class="return">返回购物车</a>
...
export default {
  ...
  methods: {
    getProductList() {
      axios.get('/static/orderList.json').then(
        result => {
          var res = result.data;
    ...
    getTotalMoney: function () {
      ...
      axios.get('/static/products.json').then((res) => {
        ...
        this.totalMoney = parseFloat(total);
      })
    }
  }
