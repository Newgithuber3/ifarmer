        ...
        <div class="cart-product">
          <table>
            ...
            <tr v-for='(item,index) in productList'>
              ...
              <td class="td-num">
                <el-input-number v-model="item.number" @change="handleChange(item.number,item.id)" :min="1" :max="10"
                                 label="描述文字"></el-input-number>
              </td>
              <td class="td-price">
                <p class="red-text"><span class="price-text">{{ item.price|formatMoney }}</span></p>
              </td>
              <td class="td-totalMoney">
                <p class="red-text"><span class="price-text">{{ item.price * item.number|formatMoney }}</span></p>
              </td>
              <td class="td-do">
                <a href="#" class="product-delete" @click='deleteOneProduct(item.id,index)'>删除</a>
              </td>
            </tr>
            </tbody>
          </table>
      ...
      <el-footer>
        ...
        <div class="cart-product-info">
          <td colspan="4" class="footer-right" style="border-right: 1px solid #ddd;">
            总计：<span style="color: red;">{{ totalMoney | formatMoney }}</span>元
            <button type="button" class="pay">去结算</button>
          </td>
        </div>
      </el-footer>
    </el-container>
  ...
  methods: {
    ...
    fetchData() {
      ...
      axios.get('/static/products.json').then(response => {
        ...
        this.productList = resp.data.pageInfo.list;
