    ...
    <el-main>
      <div class="login-main">
        <el-form  class="login-form" label-position="top" label-width="80px"
                  :rules="rules" ref="loginForm" status-icon :model="formdata">
          <h2>账号登陆</h2>
          <el-form-item label="用户名/手机号" prop="username">
            <el-input v-model="formdata.username" placeholder="请输入用户名/手机号"></el-input>
          </el-form-item>
          ...
          <el-button
            class="login-btn"
            type="danger" @click="submitForm('loginForm')">登录</el-button>
          <el-button type="text1" class="forget-password">忘记密码</el-button>
          <router-link to='/register' class="free-register">
            <el-button type="text2" @click="free-register">免费注册</el-button>
          </router-link>
        </el-form>
      </div>
    </el-main>
...
export default {
  ...
  data() {
    ...
    methods: {
      ...
      submitForm(formName) {
        var userData=JSON.stringify(this.formdata);
